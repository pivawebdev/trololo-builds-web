          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={namePt} onChange={(e) => setNamePt(e.target.value)} className="w-full bg-[#3d2c1f] border border-amber-700 rounded px-3 py-2 text-[#e8dcc5] focus:outline-none focus:border-amber-500" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-amber-300/80">Unique Name *</label>
                <input type="text" value={uniqueName} onChange={(e) => setUniqueName(e.target.value)} className="w-full bg-[#3d2c1f] border border-amber-700 rounded px-3 py-2 text-[#e8dcc5] focus:outline-none focus:border-amber-500" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-amber-300/80">Tier</label>
                <input type="number" value={tier} onChange={(e) => setTier(Number(e.target.value))} className="w-full bg-[#3d2c1f] border border-amber-700 rounded px-3 py-2 text-[#e8dcc5]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-amber-300/80">Slot Type</label>
                <input type="text" value={slotType} onChange={(e) => setSlotType(e.target.value)} className="w-full bg-[#3d2c1f] border border-amber-700 rounded px-3 py-2 text-[#e8dcc5]" placeholder="head, armor, weapon..." />
              </div>
            </div>
            <div className="flex gap-3 pt-2">
              <button type="submit" className="bg-amber-700 hover:bg-amber-600 text-[#1a120b] font-bold py-2 px-6 rounded transition-colors">{editingId ? 'Atualizar' : 'Criar'}</button>
              {editingId && <button type="button" onClick={resetForm} className="bg-gray-700 hover:bg-gray-600 text-white py-2 px-6 rounded transition-colors">Cancelar</button>}
            </div>
          </form>
        </div>
                ...
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-amber-800/20 hover:bg-[#3d2c1f]/50">
                    ...
                    <td className="p-3 space-x-2">
                      <button onClick={() => editItem(item)} className="bg-amber-600 hover:bg-amber-500 text-[#1a120b] px-3 py-1 rounded text-xs font-bold transition-colors">Editar</button>
                      <button onClick={() => deleteItem(item.id)} className="bg-red-800 hover:bg-red-700 text-white px-3 py-1 rounded text-xs transition-colors">Deletar</button>
                    </td>
                  </tr>
                ))}
              </tbody>
